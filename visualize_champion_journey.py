...
# ============================================================
# Chaos-Evolve V5 — Quantization-Aware Genetic Edition (QAGE)
...
class ObstacleEnv:
    ...
    def step(self, action):
        ...
        # Suggested fix in the environment's step function:
        if distance < 0.3:
            return self.get_state(), 0.0, True  # done=True immediately upon reaching the target, preventing spiral looping!

        progress = self.prev_distance - distance
        adjusted_distance = distance - (progress * 0.5)
        self.prev_distance = distance

        done = self.steps_taken >= 50
...
# === 6. Champion Visualization ===
def visualize_champion_journey(champion, seed=45):
    """
    Bonus feature: plots the champion robot's path and how it dodges the
    obstacle under INT4 quantization!
    """
    env = ObstacleEnv(fixed_seed=seed)
    state = env.reset()

    robot_x, robot_y = [env.agent_pos[0, 0]], [env.agent_pos[0, 1]]
    target_x, target_y = env.target_pos[0, 0], env.target_pos[0, 1]
    obs_x, obs_y = env.obstacle_pos[0, 0], env.obstacle_pos[0, 1]

    done = False
    while not done:
        # Run the champion at full INT4 QAT-compressed strength
    ...
    # Plot the path, target, and obstacle
    plt.plot(robot_x, robot_y, '-o', label='Robot Path (INT4 QAT)', color='blue')
    plt.scatter([start_x := robot_x[0]], [robot_y[0]], color='green', s=150, label='Start', zorder=5)
    plt.scatter([target_x], [target_y], color='red', marker='*', s=200, label='Target', zorder=5)

    # Plot the wall (obstacle)
    plt.vlines(x=obs_x, ymin=obs_y - 3.0, ymax=obs_y + 3.0, colors='black', linewidth=6, label='Obstacle Wall')

    plt.title(f"Chaos-Evolve V5: Champion Trajectory (Block Size = {champion.block_size})")
...
if __name__ == "__main__":
    ...
    visualize_champion_journey(champion, seed=45)